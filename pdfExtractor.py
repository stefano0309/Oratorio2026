...
def genera_note_con_tag_automatici(testo, output_dir="Note_Obsidian"):
    ...
    # Divisione flessibile basata sull'inizio della stringa "Gioco X:"
    ...
    # Pattern per la prima parte del documento (strutturata)
    tag_da_cercare = [
    ...
    for blocco in blocchi_giochi:
        ...
        # Inizializziamo il dizionario dei campi
        dati_gioco = {chiave: "" for chiave, _ in tag_da_cercare}
        
        # Controlliamo la presenza del pattern classico ("Regole:")
        strutturato = re.search(r"Regole\s*:", corpo_gioco, re.IGNORECASE)

        if strutturato:
            # --- MODALITÀ STRUTTURATA (Giochi 1-98 e conclusivi) ---
            posizioni = []
            ...
            # --- MODALITÀ FALSA STRUTTURA / TESTO CONTINUO (Gioco 99+) ---
            # Tutto il testo rimanente confluisce nella descrizione delle regole
            dati_gioco["regole"] = corpo_gioco
        ...
        # La scansione per i Tag funziona sul testo integrale, salvando anche i giochi non strutturati
        testo_per_scansione = (titolo_grezzo + " " + corpo_gioco).lower()
        ...
        if strutturato:
            ...
            # Per i giochi senza etichette (stile Gioco 99+), creiamo un layout pulito a testo continuo
            markdown_content += "## 📜 Descrizione e Svolgimento\n"
            markdown_content += f"{dati_gioco['regole']}\n"

        with open(filepath, "w", encoding="utf-8") as f:
            f.write(markdown_content)

    print(f"[COMPLETATO] Generati {len(blocchi_giochi)} file .md adattivi in '{output_dir}'.")
